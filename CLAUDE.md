...
- **Erkenning in het paspoort, QR-opbouw getest (`v2026-09-18-B`).** Nieuw in het profiel, optioneel en eenmalig: de uitgever van de erkenning (InstallQ / TloKB); daarmee gaat de erkenning als `log[].erk` mee in de QR. De QR-opbouw staat nu in `components/wkb/mkp-qr.js` (was ongetest in `WkbApp.jsx`); `tests/test-mkp-qr.js` leest de PNG terug met jsQR en vergelijkt de JSON met de invoer, plus de fasebalans. In de e-mail blijft de QR een cid-bijlage (valkuil 3), in rapport en PDF een data-URI. Het voorbeeldrapport op de landing gebruikte al `public/mkp-demo-qr.png`.
