...
```bash
npm test                  # 381 tests: normlogica, paspoort, ontsmetting, fasebalans, QR terugleestest, bronnen
```
...
**Live:** app `v2026-09-19-C` (lokaal klaar: `v2026-09-19-D`) + security-release + landing + blog (2 artikelen). Testsuite **381 tests** (`npm test`). Alles t/m `v2026-09-03-A` veldbevestigd door Martin.
...
- **Het gescande paspoort laat zien wat het draagt (`v2026-09-19-A`).** De viewer na een scan toont nu veldnotities (terugroepacties, vergeleken met `mat[]` op het toestel), de materiaallijst, per groep het beveiligende toestel, en per logregel de erkenning met controleplek, de zegels en de handtekeningstatus. De controle staat in het pakket (`mkpControleer`, `mkpVerifieer`, `mkpErkenning`, `mkpVeldnotities`, meterkastpaspoort **v0.3.1**), met de sleutel van de beheerder vastgepind in de code (`MKP_WORTEL_SLEUTEL`) in plaats van uit de index zelf. Index en feeds komen van **www**.meterkastpaspoort.nl — de 308 van het kale domein draagt geen CORS-kop. Offline: laatst opgehaalde versie uit localStorage (`components/wkb/mkp-bronnen.js`). **Sleutel van de beheerder vervangen (19-09-2026):** de oude privésleutel kwam uit een chatomgeving en was niet terug te vinden. Martin heeft lokaal een nieuw paar gemaakt; de privésleutel staat in `~/.meterkastpaspoort/wortel.pem` (buiten elke repo, offline reservekopie). Na elke wijziging van de index: `node scripts/onderteken-index.mjs onderteken ~/.meterkastpaspoort/wortel.pem` in de repo meterkastpaspoort — dat doet Martin, nooit Claude. De index wordt ondertekend zónder `handtekening` én zónder `wortel_publieke_sleutel`.
- **Erkenning in het paspoort, QR-opbouw getest (`v2026-09-18-B`).** Nieuw in het profiel, optioneel en eenmalig: de uitgever van de erkenning (InstallQ / TloKB); daarmee gaat de erkenning als `log[].erk` mee in de QR. De QR-opbouw staat nu in `components/wkb/mkp-qr.js` (was ongetest in `WkbApp.jsx`); `tests/test-mkp-qr.js` leest de PNG terug met jsQR en vergelijkt de JSON met de invoer, plus de fasebalans. In de e-mail blijft de QR een cid-bijlage (valkuil 3), in rapport en PDF een data-URI. **Fix:** een eerste scan (app nooit eerder geopend) viel terug naar het startscherm, omdat `controllerchange` de pagina herlaadde nadat het fragment al gelezen was — nu overgeslagen na een scan. **Voorbeeldrapport vernieuwd:** `public/voorbeeld-opleverrapport.html` + `.pdf` in het ontwerp uit Marketing (18-09-2026), gegenereerd met `npm run demo-rapport` (`scripts/demo-opleverrapport.mjs`) door dezelfde code als de app, met een eigen werkende QR die vóór het wegschrijven wordt teruggelezen. Niet met de hand bewerken.
- **Meterkastpaspoort v0.3: velden van een ander blijven behouden (`v2026-09-18-A`).** `mkpBouw` begon met een leeg object en wiste daarmee bij elke nieuwe QR stil de materiaallijst, `grp[].fn`/`grp[].mat`, erkenning, zegels en handtekeningen van eerdere installateurs. Nu voegt het samen met het gescande paspoort (`data.mkpImport`) via `mkpSamenvoegen` uit het pakket; het logboek wordt niet meer op acht regels afgekapt maar alleen boven de 105 modules, in de volgorde van hoofdstuk 6 (`mkpAfkappen`), met één regel in de stap Versturen. Anoniem delen haalt alleen nog `pc`/`nr`/`ean`/`ean2` uit het paspoort in plaats van het hele paspoort. `anonimiseerJob` staat nu in `veilig.js`. Doorgeklikt in de app: handtekening van de voorganger na hergebruik byte-voor-byte gelijk. De grens is **maximaal 105 modules** (besluit Martin, 18-09-2026). **Kastscan doet sinds 18-09-2026 hetzelfde:** de optionele knop "Bestaande sticker inlezen" in Documenten scant de oude QR (camera, of de link plakken) en neemt via `mkpSamenvoegen` alles mee wat Kastscan zelf niet vastlegt; een ander adres geeft een waarschuwing, geen blokkade.
...
- **Erkenningslandschap:** InstallQ erkent/certificeert (registers echteinstallateur.nl, CentraalRegisterTechniek.nl) maar wettelijke certificaten (CO/BRL 6000-25, F-gassen BRL 100/200) worden uitgegeven door CI's (Kiwa, Dekra) met eigen nummers; gasregister staat bij TloKB (tlokb.nl). Drie uitgevers, drie registers, geen koppeling — daarom toont het rapport per discipline het juiste nummer mét de juiste controleplek. **TloKB is geen uitgever:** het beheert het CO-register (co-vrijregister.nl → register.tlokb.nl/co), dat per bedrijf KVK, CI, schema en "geldig tot" toont maar **niet het certificaatnummer** — controleren gaat op bedrijfsnaam of KVK. In het paspoort: `installq:<nr>` voor de erkenning, `kiwa:<nr>` voor het CO-certificaat (bij cv-klussen, `erkVoorKlus`). Zie `docs/claude_techniek-nederland-vooronderzoek-2026-09-19.md`.
- **Meterkastpaspoort-formaat:** JSON → `deflate-raw` → base64url in het **URL-fragment** achter `meterkastpaspoort.nl/p#`, dat redirect naar `yourwkb.nl/app` (fragment reist mee). Spec v0.2 voegt `log[].erk` (`uitgever:nummer`, bv. `installq:14718`) en `grp[].fn` (de fasenummers, [1]/[2]/[3]/[1,2,3]) toe — er bestaat **geen** veld `fase` per apparaat, en ook geen apart veld voor de belasting per fase: een lezer rekent die uit `grp[].fn` + `kw`. `erk` wordt alleen geschreven als de installateur in zijn profiel de uitgever kiest (`instErkUitgever`); een nummer zonder register wordt niet gegokt. Licentie CC BY 4.0.
...
6. **Demo-QR's nooit zelf verzinnen** — gebruik `public/mkp-demo-qr.png` / `mkp-demo-sticker.png`. Een zelfgemaakte mockup-QR met ongeldige payload heeft een "storing" veroorzaakt die er geen was. De enige andere toegestane weg is die van `scripts/demo-opleverrapport.mjs`: door `mkpQrVoorRapport` zelf, en teruggelezen met een QR-lezer vóór het wegschrijven.
12. **Test een scan altijd ook als eerste bezoek** (service worker weg, caches leeg). De viewer overleefde een tweede bezoek wel en een eerste niet; wie de app al had, zag de fout nooit.
...
- Lever bij elke release: gewijzigde bestanden, testresultaat (alle tests groen, nu 381), buildresultaat, en een korte samenvatting van wat de gebruiker merkt. Meld gedragswijzigingen expliciet, ook kleine.
