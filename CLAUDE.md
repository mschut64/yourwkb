...
```bash
npm test                  # 366 tests: normlogica, paspoort, ontsmetting, fasebalans, QR terugleestest
```
...
**Live:** app `v2026-09-18-A` (lokaal klaar: `-B`) + security-release + landing + blog (2 artikelen). Testsuite **366 tests** (`npm test`). Alles t/m `v2026-09-03-A` veldbevestigd door Martin.
...
- Lever bij elke release: gewijzigde bestanden, testresultaat (alle tests groen, nu 366), buildresultaat, en een korte samenvatting van wat de gebruiker merkt. Meld gedragswijzigingen expliciet, ook kleine.
